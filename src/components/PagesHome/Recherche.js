...
export default class Recherche extends Component {
    ...
    calculerDistacne=  (long,lat,i) =>{
        ...
        fetch("https://api.mapbox.com/directions/v5/mapbox/driving/"+9.865629+","+37.270627+";"+long+","+lat+"?access_token=pk.eyJ1IjoiYWNoMDJyYWYiLCJhIjoiY2thenZpNHd5MDIybjJ5bXR3NW4zazRmbiJ9.04grBsf5HhN-BmtWVN5PXg",{
            method:"GET",
            headers: {
            ...
            .then(  (data)=>{
              let datat= data.routes[0].distance/1000;
                setTimeout(() => {
                     this.state.distance.splice(i, 0, datat);
                }, 10);
            });
    };
        ...
        return (
            ...
            <View style={styles.maincontainer}>
                ...
                <View style={styles.recherchetitle}>
                    ...
                    {
                        this.state.one !== ''?
                            <Text style={styles.text01}>{this.state.one}</Text>
                            :
                    ...
                    {
                        this.state.three !=='' ?
                                    <Text style={styles.text01}>{this.state.two+','+this.state.three}</Text>
                                :
                                    <Text style={styles.text01}>{this.state.two}</Text>
                                    ...
                                    <ScrollView  style={{ flex: 1 }}
                                                 ...
                                                 nestedScrollEnabled = {true}>
                                        {
                                            item.timeplan.jours.map((itzm, k) => (
                                                itzm.jour.toString() === startDate.concat(' (CET)').toString() ? (
