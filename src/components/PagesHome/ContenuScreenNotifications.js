...
import {
    Text,
    View,
    ScrollView, TouchableOpacity, Dimensions, StyleSheet, Animated, Image,
} from 'react-native';
                ...
                <ScrollView>
                        ...
                        this.state.list.map((item, i) => (
                            <TouchableOpacity key={i} style={{backgroundColor:this.state.colorbacks[i].index}} onPress={() =>{ this.sendCred(i).then("err");} }  onLongPress={()=>{this.setState({content:true ,notifid:i})} } >
                                <Notification key={i} NotificationType={item.type} Doctorname ={item.doctornom} Time={ item.time} image={item.picdoc}  />
                            </TouchableOpacity>
